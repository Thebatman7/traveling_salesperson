...
class Proj5GUI( QMainWindow ):
	...
	def displaySolution( self ) :						# what about calling this somehow every time a new bssf is found?
		...
		if self._solution:
			...
			if edges:
				edgeColor  = (0,255,0) #(128,128,255)
				labelColor = (0,20,25) #(64,64,255)
				for edge in edges:
					pt1,pt2,label = edge
					if(label == -77777):
						wrongedgeColor  = (199,0,57) #(128,128,255)
						wronglabelColor = (88,24,69) #(64,64,255)
						self.view.addEdge( QPointF(pt1._x,pt1._y), \
									   QPointF(pt2._x,pt2._y), \
									   '{}'.format(label), wrongedgeColor, wronglabelColor )
					else:
						self.view.addEdge( QPointF(pt1._x,pt1._y), \
											QPointF(pt2._x,pt2._y), \
